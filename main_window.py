# main_window.py - главное окно приложения (исправленная версия с правильными путями к данным)
import os
import sys
import re
import subprocess
...
class DocumentWorker(QThread):
    ...
    def __init__(self, save_root, fields, templates_dir):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            if not os.path.exists(templates_dir):
                self.error.emit(f"Папка с шаблонами не найдена: {templates_dir}")
                return
...
class MainWindow(QMainWindow):
    ...
    def get_script_dir(self):
        """Возвращает директорию, где находится исполняемый файл программы (Программа.exe или .py)"""
    ...
    def get_data_dir(self):
        ...
        - если программа скомпилирована и запущена через загрузчик (есть папка _internal),
          то это родительская папка (где лежит DocumentFiller.exe)
        - если запущена как скрипт (разработка), то это папка проекта (где лежит main.py)
        - иначе (другие случаи) – папка текущего исполняемого файла
        """
        if getattr(sys, 'frozen', False):
            # Скомпилированная версия
            exe_dir = os.path.dirname(sys.executable)
            # Проверяем, находимся ли мы внутри папки _internal
            if os.path.basename(exe_dir) == "_internal":
                return os.path.dirname(exe_dir)
            else:
                return exe_dir
        else:
            # Режим разработки: возвращаем папку проекта (где лежит main.py)
            return os.path.dirname(os.path.abspath(__file__))

    def get_template_dir(self):
        """
        Возвращает путь к папке с шаблонами.
        При необходимости копирует шаблоны из внутренней папки в data_dir.
        """
        data_templates = os.path.join(self.get_data_dir(), "Шаблоны")
        internal_templates = os.path.join(self.get_script_dir(), "Шаблоны")  # внутри _internal

        # Если в data_dir нет папки Шаблоны, но есть во internal – копируем
        if not os.path.exists(data_templates) and os.path.exists(internal_templates):
            try:
                shutil.copytree(internal_templates, data_templates)
                print(f"📁 Шаблоны скопированы в {data_templates}")
            except Exception as e:
                print(f"❌ Ошибка копирования шаблонов: {e}")
                # Если не удалось скопировать, используем внутренние (но они могут быть недоступны после первого запуска)
                return internal_templates
        return data_templates if os.path.exists(data_templates) else internal_templates

    def get_excel_file_path(self):
        """Получить путь к Excel файлу (рядом с DocumentFiller.exe)"""
        return os.path.join(self.get_data_dir(), "анкеты_данные.xlsx")

    def get_default_save_folder(self):
        """Получить папку для сохранения по умолчанию (рядом с DocumentFiller.exe)"""
        return os.path.join(self.get_data_dir(), "документы")
    ...
    def create_documents(self):
        ...
        try:
            ...
            templates_dir = self.get_template_dir()
            if not os.path.exists(templates_dir):
                QMessageBox.critical(self, "Ошибка", f"Папка с шаблонами не найдена: {templates_dir}")
                return

            self.progress_bar.setVisible(True)
            self.worker = DocumentWorker(save_root, values, templates_dir)
