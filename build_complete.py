...
def create_missing_configs():
    ...
    configs = {
        'version_config.json': {
            ...
            "update_url": "",
            "check_updates_on_start": True,  # Включено по умолчанию
...
def check_final_distribution(dist_dir):
    ...
    # Создаем README с инструкциями по обновлению
    create_update_readme(dist_dir)


def create_update_readme(dist_dir):
    """Создать README с инструкциями по обновлению"""
    readme_content = '''ВАЖНЫЕ ИНСТРУКЦИИ ПО ОБНОВЛЕНИЮ

⚠️ ПРЕДУПРЕЖДЕНИЕ О БЕЗОПАСНОСТИ:

Перед обновлением программы обязательно:
1. Создайте резервную копию ваших данных
2. Убедитесь, что у вас есть активационный ключ лицензии
3. Сохраните важные документы в надежном месте

📁 ВАЖНЫЕ ФАЙЛЫ ДЛЯ СОХРАНЕНИЯ:
- анкеты_данные.xlsx (база данных анкет)
- license.json (файл лицензии) 
- Шаблоны/ (ваши шаблоны документов)
- документы/ (созданные документы)

🔧 ЕСЛИ ОБНОВЛЕНИЕ НЕ УДАЛОСЬ:

1. Не паникуйте! Ваши данные в безопасности
2. Запустите программу снова - она автоматически восстановится из резервной копии
3. Если проблема persists, удалите папку программы и восстановите из резервной копии
...
'''
    ...
    readme_path = os.path.join(dist_dir, 'ВНИМАНИЕ_ОБНОВЛЕНИЕ.txt')
    with open(readme_path, 'w', encoding='utf-8') as f:
        f.write(readme_content)
    print("📖 Создан файл с предупреждением об обновлении")
...
'''
    ...
    with open('document_filler.spec', 'w', encoding='utf-8') as f:
        f.write(spec_content)
    print("Создан spec файл для сборки")
